...
chai.Assertion.prototype.assert = function (expr, msg, negateMsg, expected, _actual, showDiff) {
    ...
    if (!ok) {
        const actual = _actual !== undefined ? _actual : chai.util.flag(this, 'object');
        ...
        if (actual instanceof Node || expected instanceof Node) {
            ...
            throw new chai.AssertionError(message, { actual: fmt(actual), expected: fmt(expected), showDiff: false }, chai.util.flag(this, 'ssfi'));
        }
    }
    return origAssert.call(this, expr, msg, negateMsg, expected, _actual, showDiff);
};
import { resetMouse, sendMouse } from '@web/test-runner-commands';
import { identity } from '@openscd/scl-lib';
import { XMLEditor } from '@omicronenergy/oscd-editor';
import { busSections, makeBusBar } from './foundations/connectivity.js';
import { getSLDAttributes, setSLDAttributes, } from './foundations/sld-attributes.js';
...
import { findSubstationSvgRoot, gridPosToViewportCoords, sclLabelToViewportCoords, svgToViewportCoords, vlOrigin, vlResizeBR, placeTL, placeBR, eqPos, eqTarget, } from './test-helpers.js';
...
describe('SLD Editor', () => {
    ...
    function contextMenu() {
        return (getSldSubstationEditor(element)
            .shadowRoot.querySelector('sld-context-menu')
            ?.shadowRoot?.querySelector('oscd-menu') ??
            null);
    }
    function clickInteractive(element) {
        const target = element.shadowRoot?.querySelector('#button, #item') ??
            element;
        if (target !== element) {
            target.click();
        }
    ...
    function contextMenuItems() {
        return Array.from(contextMenu()?.querySelectorAll('oscd-menu-item') ?? []);
    }
    ...
    async function waitForMenuClose() {
        await waitUntil(() => contextMenuItems().length === 0, 'context menu items did not clear');
    }
    function gridPos(gx, gy) {
        return gridPosToViewportCoords(findSubstationSvgRoot(getSldSubstationEditor(element)), gx, gy);
    }
    function labelPos(lx, ly) {
        return sclLabelToViewportCoords(findSubstationSvgRoot(getSldSubstationEditor(element)), lx, ly);
    }
    function svgPoint(svgX, svgY) {
        return svgToViewportCoords(findSubstationSvgRoot(getSldSubstationEditor(element)), svgX, svgY);
    }
    beforeEach(async () => {
        ...
        @oscd-sld-edit-scl=${({ detail: { element: e }, }) => {
            lastCalledSclEdit = e;
        }}
    ...
    describe('given a voltage level', () => {
        ...
        it('forbids undersizing the substation', async () => {
            ...
            sldSubstationEditor.shadowRoot
                ?.querySelector('oscd-text-button[slot="primaryAction"]')
                ?.shadowRoot?.querySelector('#button')
                ?.click();
        ...
        it('requests voltage level scl edit on edit menu item select', async () => {
            ...
            await sldSubstationEditor.updateComplete;
            expect(lastCalledSclEdit).to.equal(element.doc.querySelector('VoltageLevel'));
        });
        ...
        it('moves the voltage level label on label left click', async () => {
            ...
            const svg = findSubstationSvgRoot(getSldSubstationEditor(element));
            const rect = svg.getBoundingClientRect();
            const vb = svg.viewBox.baseVal;
            const clickX = Math.round(rect.left + ((3.9 - vb.x) / vb.width) * rect.width);
            const clickY = Math.round(rect.top + ((3.4 - vb.y) / vb.height) * rect.height);
    ...
    describe('given IED references', () => {
        ...
        it('triggers oscd-scl-dialogs on IED label middle-click', async () => {
            await placeIedFromScl('IED1', 10, 10);
            const sclDialogs = element.shadowRoot?.querySelector('oscd-scl-dialogs');
        ...
        it('renames an IED via context menu edit dialog', async () => {
            ...
            const menu = contextMenu();
            expect(menu).to.exist;
            const editItem = contextMenuItems().find(item => item.textContent?.includes('Edit'));
            expect(editItem).to.exist;
            clickInteractive(editItem);
            await settle();
    ...
    describe('given conducting equipment', () => {
        ...
        it('requests equipment scl edit on edit menu item select', async () => {
            ...
            clickInteractive(menuItem(-2));
            await sldSubstationEditor.updateComplete;
            expect(lastCalledSclEdit).to.equal(element.doc.querySelector('[type="SMC"]'));
        });
            ...
            describe('between more than two pieces of equipment', () => {
                ...
                describe('and a bus bar', () => {
                    ...
                    it('requests bus bar scl edit on edit menu item select', async () => {
                        ...
                        clickInteractive(menuItem(-2));
                        await sldSubstationEditor.updateComplete;
                        expect(lastCalledSclEdit).to.equal(element.doc.querySelector('[name="BB1"]'));
                    });
