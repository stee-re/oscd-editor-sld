...
import { isEqType, ringedEqTypes, singleTerminal, } from '../../foundations/equipment.js';
import { newPlaceEvent, newRotateEvent, newSelectEvent, newStartConnectEvent, newStartPlaceEvent, } from '../../foundations/events.js';
...
function equipmentRenderState(equipment, context, { preview = false, connect = false } = {}) {
    if (context.placing === equipment && !preview) {
        return undefined;
    }
    if (context.connecting?.from.closest('Substation') === context.substation &&
        !connect) {
        return undefined;
    }
    ...
    return {
        ...
        canShowBottomPort: !(bottomTerminal ||
            context.resizingBR ||
            context.resizingTL ||
            context.connecting ||
        ...
        canShowTopPort: !(topTerminal ||
            ...
            placingOtherElement ||
            context.disabled),
        clickthrough: connect ||
            (!context.idle && context.placing !== equipment) ||
            (context.disabled && !selectable),
        diagramElementId: equipment.closest('Substation') === context.substation
            ? `${identity(equipment)}`
            : undefined,
        ...
        position: [x, y],
        selectable,
        showBottomConnectIndicator: !!context.connecting &&
            !connectingFromSelf &&
            !(context.mouseX === x &&
                context.mouseY === y &&
            ...
            !singleTerminal.has(eqType) &&
            !context.disabled,
        showTopConnectIndicator: !!context.connecting &&
            !connectingFromSelf &&
            !(context.mouseX === x &&
                context.mouseY === y &&
...
function equipmentRenderActions(equipment, context, state) {
    let handleClick = (e) => {
    ...
    if (context.placing === equipment) {
        ...
        const parent = Array.from(context.substation.querySelectorAll(':root > Substation > VoltageLevel > Bay')).find(bay => !isBusBar(bay) && containsRect(bay, x, y, 1, 1));
        if (parent && canPlaceAt(context.substation, equipment, x, y, 1, 1)) {
            handleClick = () => {
                context.dispatch(newPlaceEvent({
                    x,
                    y,
                    element: equipment,
                    parent,
    ...
    return {
        ...
        onStartBottomConnect: () => {
            context.dispatch(newStartConnectEvent({
                from: equipment,
                fromTerminal: 'T2',
...
function renderEquipmentPreviewLabels(equipment, context, { preview = false } = {}) {
    return preview
        ? svg `<g class="preview">
        ${[
            context.renderLabel(equipment, { preview }),
            ...Array.from(equipment.querySelectorAll('Text')).map(text => context.renderLabel(text, { preview })),
        ]}
      </g>`
        : nothing;
}
function renderEquipment(equipment, state, actions) {
    const [x, y] = state.position;
...
export const conductingEquipmentArtifact = {
...
//# sourceMappingURL=conducting-equipment.js.map