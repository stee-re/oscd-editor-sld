...
function sections(element) {
    ...
    return Array.from(privates).flatMap(priv => Array.from(priv.children).filter(child => child.localName === 'Section' && child.namespaceURI === sldNs));
}
...
export function connectivityPath(element, ...rest) {
    const pedigree = [];
    let child = element;
