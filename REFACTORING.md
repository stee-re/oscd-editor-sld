...
- [ ] **Collapse the upward `start*` gesture events into a single interaction-start event.** The five artifact-emitted events `newStartPlaceEvent`, `newStartPlaceLabelEvent`, `newStartResizeBREvent`, `newStartResizeTLEvent`, `newStartConnectEvent` are the un-collapsed mirror of the `Interaction` union — each maps 1:1 to a mode, and they are dispatched from ~13 call sites across the five `drawing/artifacts/*` files. Replace them with one `newStartInteractionEvent({ mode, element, terminal?, offset? })` so the view→controller channel speaks the same discriminated union the controller already owns. **Removes 4 events.** This is the _upward_ half of unifying the artifact↔`Interaction` channel (item 3 is the downward half); it touches every artifact and its spec, so slice per mode.
- [ ] **Collapse the downward artifact interaction projections — artifacts read `interaction` directly.** `SldSubstationViewer` already receives one canonical `.interaction`, but artifact contexts are still handed projected fields (`placing`, `placingLabel`, `resizingBR`, `resizingTL`, `connecting`, `idle`). Migrate artifacts to consume `interaction` with mode-narrowing and delete the projection plumbing. The _downward_ half of unifying the artifact↔`Interaction` channel (pairs with item 2). Touches many render/action helpers and their specs — do carefully.
- [ ] **Split `SldEditor`'s interaction lifecycle into explicit methods.** The `start*` methods still call the catch-all `reset()` — a holdover from when state lived in several fields. With the `Interaction` union, assigning a new value already replaces the previous mode, but `reset()` _also_ cancels the placement promise and emits `sld-editor-in-action`. Separate the concerns into `setInteraction(...)`, `cancelInteraction()`, and `finishInteraction()` so replacement, cancellation, and completion read distinctly. Do together with item 6 (that event is emitted from this path).
- [ ] **Replace the bare-boolean `sld-editor-in-action` event with a typed interaction-change event.** The detail is currently just `true` / `false`, which says nothing about _what_ changed or _which_ mode is active. Audit consumers first (currently only `oscd-editor-sld.ts`); if it is not public API, emit `sld-editor-interaction-changed` with `{ active, mode }` (or `{ interaction }`). Pairs with item 5, which owns the emit sites.
...
- `npm run test` passed with `517 passed, 0 failed` (after the header hoist).
- Substation header hoisted out of the viewer into an editor-owned `<sld-substation-header>` slotted into `<sld-substation-viewer>`; `export.ts` split into viewer-owned `serializeForExport` + editor-triggered `downloadSvg`. Two intent event types removed (`oscd-sld-resize-substation`, `oscd-sld-delete-substation`); `oscd-sld-edit-scl` retained (still authored by the context menu and `label.ts`). The viewer now emits only diagram-gesture intents.
