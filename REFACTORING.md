...
- `npm run test` passed with `526 passed, 0 failed` (after extracting `connectPreviewElbow`; +4 geometry specs).
- Connect-preview elbow geometry extracted to the pure `foundations/geometry.ts` `connectPreviewElbow(path, cursor, snap?)`; `renderConnectionPreviewLayer` now keeps only snap-target resolution, SVG generation, and event dispatch, and the preview and the committed path share the same corner/far/near points.
