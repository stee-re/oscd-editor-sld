...
- [ ] **Dissolve `SldEditor`'s `reset()` / `start*` lifecycle wrappers — a mode change _is_ the transition.** _(Re-scoped 2026-07-02, superseding the earlier "split `reset()` into `setInteraction`/`cancelInteraction`/`finishInteraction`" framing — see design note.)_ The `start*` methods still call the catch-all `reset()`, a holdover from when state lived in several fields. But with the `InteractionState` union, exclusivity is intrinsic: **entering any mode already _is_ leaving the previous one**, and `idle` is simply "the mode with no work attached". The "why" of going idle (finished vs cancelled) is not state — nothing downstream branches on it — so `setInteraction`/`finishInteraction`/`cancelInteraction` would be three names for one operation: **assign**. Target end-state: delete `reset()` and the `startResizingBottomRight/TopLeft`, `startPlacingLabel`, `startConnecting` wrappers; call sites just set `this.interaction = interactions.<mode>(…)` (Escape → `interactions.idle()`). Keep `startPlacing` **only** because it returns a Promise (see below). This absorbs item ↓ (the `sld-editor-in-action` event) since that becomes _derived_, not orchestrated.
  - **Design note (2026-07-02) — the two genuine side-effects, made reactive.** `reset()` does three things: (1) `interaction = idle()` — pure assignment; (2) `_resolvePlacement?.(undefined)` — resolve a dangling placement promise; (3) emit `sld-editor-in-action`. Only (1) is state. (2) and (3) are _consequences of a transition_ and should be driven reactively (Lit `willUpdate`/`updated` on the `interaction` `@state`), not hand-orchestrated in every wrapper — which is exactly the redundancy (each `start*` currently emits `in-action:false` via `reset()` then `in-action:true`, a flap):
    - **in-action** is purely derived: `active = interaction.mode !== 'idle'`; emit on change.
    - **placement promise** is the _only_ place "why" is observable: a successful place resolves with `{element,parent,x,y}`, anything else with `undefined`. This is already handled locally in `placeElement` (`src/sld-editor.ts` ~405–414), which **captures the resolver and clears `_resolvePlacement` _before_ transitioning**, then resolves with the payload. So the reactive rule is trivially "on leaving `placing`, resolve any still-pending placement promise with `undefined`"; `placeElement` opts out by clearing it first. No finish/cancel distinction needed at the state level.
  - **Open question 1 (assignment mechanism).** Either (a) a single `setInteraction(state)` funnel as the one assignment point (so the reactive side-effects have one home and we don't rely on `@state` setter semantics across scattered ad-hoc assignments; also a natural place for the old `if (element)` guards some wrappers had), or (b) truly raw `this.interaction = …` everywhere, driving side-effects off Lit's update lifecycle. Leaning (a).
  - **Open question 2 (event payload).** Given in-action becomes derived, decide with the item below whether to keep the bare boolean the host already consumes, or emit the typed `{ active, mode }`.
  - **Consumers (audited 2026-07-02):** `sld-editor-in-action` has exactly one consumer, `oscd-editor-sld.ts:208` (sets `this.inAction`, forwarded to `<sld-toolbar>.inAction`). Not public API → safe to reshape.
- [ ] **Replace the bare-boolean `sld-editor-in-action` event with a typed interaction-change event.** _(Note 2026-07-02: largely absorbed by the item ↑ — once in-action is derived from `interaction.mode !== 'idle'`, this is just a payload-shape decision. Keep bare boolean unless a consumer needs the mode.)_ The detail is currently just `true` / `false`, which says nothing about _what_ changed or _which_ mode is active. Audit consumers first (currently only `oscd-editor-sld.ts`); if it is not public API, emit `sld-editor-interaction-changed` with `{ active, mode }` (or `{ interaction }`). Pairs with the item above, which owns the emit sites.
...
- [ ] **Split `renderConnectionPreviewLayer` (~76 lines).** The largest remaining `render*` method in the viewer; extract the connection-polyline geometry into a small pure helper (matching the `foundations/geometry.ts` convention).
