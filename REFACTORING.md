...
1. **Optimize idle mouse rendering.** Gesture-start handlers now read live
   event-time coordinates. Next, add `shouldUpdate` to skip idle
...
- [ ] **Skip idle mouse-coordinate-only re-renders.** Today the substation editor keeps `mouseX/mouseY/mouseX2/mouseY2/mouseX2f/mouseY2f` as reactive `@state`, updated on every `mousemove`. This stored state is **load-bearing during an active drag** — it's what makes Lit re-render so the ghost follows the cursor (no click involved), so it is *not* merely "stored just in case". The waste is specifically re-rendering **while idle**. The former staleness source has been removed: gesture-*start* handlers now compute their coordinate input from the live click event via shared `gridPosition(event)` / `halfGridPosition(event)` context helpers. Synthetic zero-coordinate tests still fall back to stored mouse state to preserve existing test ergonomics, but real pointer events use event-time coordinates.
  ...
  - **Step 2 (payoff):** add `shouldUpdate` to skip re-renders while `idle` when only the mouse-coordinate state changed.
