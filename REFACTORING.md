...
- [-] **Give `sld-substation-viewer` an opt-in fit-to-container (scale-to-fit) mode for standalone use.** _(Parked 2026-07-08 — **won't do until needed**; kept as a documented backlog item, not an active task. Containment is already solved by the completed `overflow: auto` default below, so nothing today forces this; revisit when a real standalone consumer needs "see the whole diagram scaled to fit" — most likely alongside the `sld-viewer` facade.)_ The viewer paints its `svg#sld` at **intrinsic resolution** — `width = w × gridSize`, `height = h × gridSize` (e.g. a 50×25 substation at `gridSize` 32 → a fixed 1600×800px svg). **Containment is now handled** by a default `section { overflow: auto }` on the viewer's shadow wrapper (see the completed entry below): the svg no longer spills out of a constrained container — it scrolls within it. That default is deliberately safe because `overflow: auto` is **dormant unless an ancestor imposes a definite width below the content**: verified in the live plugin that `main { width: fit-content }` still expands to the svg's native 1600px and the section shows **no** scrollbar (`scrollWidth === clientWidth`), so the shell keeps owning the scroll/zoom viewport exactly as before; only a _constrained_ standalone consumer (e.g. a fixed-width card) activates the scroll. **What remains open is _fit_, not _containment_:** scrolling lets you reach the rest of a too-large diagram, but not _see the whole substation at once_ scaled down. Add an **opt-in** fit mode (a `fit` / `scale-to-fit` boolean, or a sizing strategy prop), **off by default** to preserve the native-size/zoom contract, that lets the svg scale to its container via the viewBox (already clean — drop the fixed pixel `width`/`height` and let CSS `width:100%; height:auto` drive it when the flag is set; note a blanket `max-width:100%` would break the shell's gridSize-based zoom, so it must be opt-in). Likely owned by / co-designed with the `sld-viewer` facade (above), since read-only standalone display is precisely where scale-to-fit matters; once it exists, `viewer-demo.html` would enable it instead of showing native resolution.

- [-] **Introduce an `sld-viewer` facade — first step of the §7 viewer/editor split.** _(Investigated 2026-07-08 — **shelved as "not worth the pain" for now**; kept here as a documented decision, not an active task.)_ Today there is no read-only, multi-substation viewer: `sld-substation-viewer` is per-`Substation`, and the only thing that maps `:root > Substation` → one-viewer-each is `sld-editor` (read-write). The attraction was twofold: **(1) discoverability** — a consumer who sees `sld-editor` reasonably expects an `sld-viewer` counterpart — and **(2) removing the smell** that `sld-editor` owns the substation loop at all. **Why it was shelved:** only the _composition_ shape (`sld-editor` renders `<sld-viewer>` and layers interaction/edit on top) actually delivers (2) — a mere _parallel_ sibling would duplicate the loop into two places, which is a worse smell than the one it fails to remove. And composition is blocked by a genuine, inelegant obstacle: **the editor slots a per-`Substation` `sld-substation-header`** (edit / resize / delete / export chrome) into each viewer's `<slot name="header">`. If the facade owned the loop, the editor could no longer slot per-substation without either a `.headerRenderer=${(sub)=>…}` function-prop or per-substation named slots — both uglier than the status quo. (The ~11 per-substation event handlers are _not_ the blocker: they are all `bubbles + composed`, so they would cross a facade boundary for free; and `selectable`/`highlight`/`oscd-sld-selected` are already pure viewer-tier concerns the editor just passes through.) **Revisit only if** the header-slotting composition problem gains an elegant answer (e.g. the header migrates in a way that removes per-substation editor slotting), at which point the composition — and with it the discoverability win _and_ smell removal together — becomes worthwhile. ARCHITECTURE.md §7 still describes the intended long-term end-state.

- [x] **Route placement-copy through the intent channel (centralise the clone helper + correct the view/editor split). DONE.** Copy affordances (shift-click on an artifact; the context-menu "Copy" items) used to **manufacture the preview clone _in the caller_** and hand the already-cloned element up as the `element` of a `placing` intent: `copyElementForPlacement` (deep-clone + strip foreign IED refs/connectivity + re-UUID terminals via `setSLDAttributes`) was invoked at **7 sites** — 4 in the rendering tier (`drawing/artifacts/{conducting-equipment,equipment-container,power-transformer}.ts`, the actual tier-boundary smell) and 3 in the editor-tier context-menu factory. **The real payoff isn't "tier smell" — it's centralisation + a correct conceptual split:** the dangerous, easy-to-get-wrong clone helper is now invoked from exactly **one** place, and the responsibility is split along the natural seam — _deciding_ to copy (shift-key, "Copy" menu item) is a **view/UI** concern carried by the intent; _constructing_ the clone (UUID surgery) is an **editor** concern. **Fix (sparred + greenlit at full scope):** (1) `InteractionIntent`'s `placing` variant gained an optional `copy?: boolean` — callers now pass `{ mode: 'placing', element: <source>, copy: e.shiftKey }` (artifacts) or `copy: true` (factory) and never touch `copyElementForPlacement`. (2) `SldEditor.startInteraction`'s `placing` case does the clone: `intent.copy ? copyElementForPlacement(intent.element, this.nsp) : intent.element`. `startPlacing` stays clone-agnostic (host bay-typical + tests pass ready elements). (3) Because clone construction is now editor-only, `copyElementForPlacement` **moved out of viewer-tier `foundations/sld-placement.ts`** into a new single-responsibility editor-tier module `foundations/placement-clone.ts` — `sld-placement.ts` is now purely the viewer-safe placement/resize **validation** rules (`canPlaceAt`/`canResizeTo`/`canResizeToTL`), shedding its `uuid`/`getSLDAttributes`/`setSLDAttributes` imports. (4) **Dead-code cleanup enabled by the change:** the per-artifact context `nsp: string` field (only ever read to feed the now-removed clone call) was removed from all three artifact context interfaces + their three viewer construction sites (`sld-substation-viewer.ts`) + `test-context.ts`. The viewer's public `get nsp()` getter is **kept** (doc-derived convenience used by a test); the context-menu context's `nsp` is **kept** (still feeds flip/add-text/update-attribute edits). Per the code-structure test-co-location rule the `copyElementForPlacement` `describe` block moved with the function into new `foundations/placement-clone.spec.ts`. Verified `tsc --noEmit` clean, `npm run test` (`572 passed, 0 failed`; +3 net: `events.spec` copy-flag, editor `startInteraction` clones-on-copy / places-source-otherwise; the two artifact "copy on shift-click" specs updated to assert `element === source` + `copy === true` rather than a pre-cloned element), `npm run format` (only the 2 pre-existing `geometry.ts` tsdoc warnings).

- [x] **Route toolbar-initiated placement through the intent channel (interaction ownership). DONE.** The `<sld-toolbar>` is rendered by the **host** (`oscd-editor-sld.ts`) as a **sibling** of `<sld-editor>`, but placement is an _interaction_ and therefore an editor concern. The host was reaching **imperatively** into the editor — `this.sldEditor?.startPlacing(element)` and, worse, `this.sldEditor.interaction = idle()` in a host `reset()` — directly assigning the editor's reactive interaction state and splitting ownership of the interaction lifecycle host↔editor. Investigation also found the host's own `window` Escape handler was **redundant** with the editor's (the editor already handles Escape→idle itself). **Fix (sparred + greenlit):** (1) the editor gained one public `startInteraction(intent)` (promoted from the private `handleStartInteraction`; its own view listeners call it too) — a single entry for every begin-transition that returns the placement **promise** for the `placing` case; and `cancelInteraction()`, so callers _request_ an abort rather than assigning `interaction`. (2) The toolbar (`sld-toolbar.ts`, `sld-ied-menu.ts`) now emits the shared `oscd-sld-start-interaction` intent (killed the bespoke `start-placing` event) — exactly like the context-menu and artifacts. (3) The host became a **thin bridge**: it listens for `oscd-sld-start-interaction` **on the toolbar element only** (so the editor's own bubbling intents aren't double-handled) and forwards the opaque intent via `startInteraction`; `@cancel` → `cancelInteraction()`; the bay-typical flow `await`s `startInteraction({ mode: 'placing', element: bayTypical })` then inserts IEDs (document mutation stays the host's job). Deleted the host `startPlacing`/`reset`/`handleKeydown` + its redundant `window` Escape listener, and the now-unused `idle` import. Net: **the host never assigns `interaction`; the editor is its sole owner; toolbar placement uses the same channel as every other interaction.** _(Design note: `startInteraction` takes an `InteractionIntent` (view→controller, begin-only) built as a raw literal — the established convention — **not** an `InteractionState` from `interactions.placing()`; the two types are deliberately distinct and opposite-flowing.)_ Verified `tsc --noEmit` clean, `npm run test` (`569 passed, 0 failed`; +5 specs: editor `startInteraction`/`cancelInteraction` contract incl. the promise-resolves-on-leave case, host toolbar-cancel abort; updated toolbar/ied-menu specs to the intent event), `npm run format` (only the 2 pre-existing `geometry.ts` tsdoc warnings). `ARCHITECTURE.md` already described this clean design, so it now matches reality. _(Test-timing gotcha recorded: setting `interaction` to `placing` then `idle` **synchronously** is coalesced into one Lit update, so `updated()`'s recorded "previous" is the pre-batch value and the leave-`placing` promise never resolves — a test must `await updateComplete` between the two, mirroring real usage where placement resolves in a later task.)_
...
- [x] **Expose themable artifact colours as CSS variables. DONE.** The doc's original example — _"the equipment top-indicator `#BB1326`"_ — was already resolved by the earlier theming workstream: the rendered diagram is fully tokenised (e.g. `conducting-equipment.ts` draws the top/bottom indicators with `var(--oscd-sld-terminal-color)`). The only remaining literals were in `context-menu/sld-context-menu-factory.ts`, and they split into two categories that were treated differently. **(A) Themeable chrome** — the Delete IED destructive-action colour, which writes nothing to the document — now references theme tokens (`var(--md-sys-color-error, var(--oscd-error))`), and `sld-context-menu.ts` gained `sldThemeStyles` so tokens resolve standalone (matching every other component). **(B) Fixed document-data swatches** — the "Red"/"Blue" label commands are a fixed-value colour picker: each `updateSLDAttributes(text, …, { color })` writes a literal hex into the SCL `<Private>` SLDAttributes, and a coloured label renders with that literal (`label.ts`: `fill: ${attributes(el).color}`), NOT via a token. So a single constant (`SLD_RED`/`SLD_BLUE`) drives all three coupled uses — the show/hide guard, the persisted write, **and the menu-item swatch preview** — guaranteeing the swatch always matches what the command writes and renders. These must stay literal (portable across IEC 61850 tools, identical in unthemed export, theme-independent) and must **not** use the themeable `--oscd-sld-red`/`--oscd-sld-blue` diagram tokens, which retint the _brand_ and may by design diverge from these data swatches. The label default `#000` moved to `DEFAULT_LABEL_COLOR` in `sld-attributes.ts` (its read-default), shared with the "Reset Color" no-op check. _(Correction during review: an earlier pass mistakenly themed the Red/Blue swatches with the diagram tokens, which would let the swatch preview drift from the literal it writes/renders under a re-themed brand; reverted so the constant is the single source for guard + write + swatch.)_ Behaviour-preserving. Verified `tsc --noEmit` clean, `npm run test` (`551 passed, 0 failed`), `npm run format` (only the 2 pre-existing `geometry.ts` tsdoc warnings).
...
- `src/foundations/sld-placement.ts` — SLD placement/resize validation rules (`canPlaceAt`, `canResizeTo`, `canResizeToTL`). Pure, read-only, **viewer-tier safe** — no clone construction, no `EditV2`.
- `src/foundations/placement-clone.ts` — `copyElementForPlacement` (pure preview-clone: strips foreign connectivity/IED refs and re-UUIDs terminals). Returns `Element`, never `EditV2`, but is **editor tier**: invoked only by `SldEditor.startInteraction` when a `placing` intent carries `copy: true`.
...
- `npm run format` passed (only the 2 pre-existing `geometry.ts` tsdoc warnings); `tsc --noEmit` clean.
- `npm run test` passed with `552 passed, 0 failed` (redesigned the legacy-coordinate migration screen: `oscd-editor-sld.ts` `hasOldNamespace` branch now renders a centered, no-chrome `.migration` block — `oscd-icon` shield + reassuring heading/lead + `<details>` disclosure + `oscd-filled-button` CTA — replacing the bare top-left `oscd-text-button`. Registered `oscd-icon`/`oscd-filled-button` in `scopedElements`; the 4 convert-button specs + import switched from `OscdTextButton`/`oscd-text-button` to `OscdFilledButton`/`oscd-filled-button`, exact `Convert SLD Layout` label preserved).
- `npm run format` passed (only the 2 pre-existing `geometry.ts` tsdoc warnings); `tsc --noEmit` clean.
