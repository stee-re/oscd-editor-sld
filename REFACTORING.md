...
- [ ] **Make the connect-path growth immutable (remove in-place array mutation).** While connecting, the path is grown by mutating the `Interaction`'s `path` array in place and calling `requestUpdate()` by hand (`sld-substation-viewer.ts` `renderConnectionPreviewLayer`, `drawing/artifacts/connectivity-node.ts`). This works only because Lit `@state` does not detect intra-array mutation — hence the manual `requestUpdate`. Replace with immutable reassignment (`this.interaction = { ...connecting, path: [...] }`, the shape the deleted `appendConnectionPoint` helper modelled) so reactivity is automatic and the value is never mutated behind the controller's back. Behaviour-sensitive — needs care around the click/preview ordering; guard with the existing connect specs.
