...
- [ ] **Add direct, self-documenting `guard`-contract tests to `sld-substation-viewer.spec.ts`.** The file is currently benchmark-only. The base-layer `guard` memoization (its `baseLayerKey`) is a subtle invariant: drop a key item and the base layers silently go **stale** (fast but wrong). Its behaviour is _already_ covered **indirectly** by `sld-editor.spec.ts` — resize commit-value tests (`allows resizing bays` etc.) exercise the `resizingBR` mouse-tracking branch because the committed `w`/`h` is baked into the render-time click closure; connect tests cover `connectingFrom`; placing tests cover the placed-element suppression; move/edit tests cover doc-change invalidation. So this is **not a correctness hole** — it is a _directness_ gap: a broken key currently fails an editor test with a confusing symptom (wrong committed width) instead of an obvious "the guard froze the base layers" signal, and the view-flag/highlight key items are not clearly pinned anywhere. Worth a small isolated set on the viewer in isolation: (a) mutating the doc + bumping `docVersion` updates the base-layer DOM; (b) entering `placing` removes the placed element from the base layers; (c) `resizingBR`/`connectingFrom` re-render on cursor move while `placing`/`idle` do not; (d) toggling `showLabels`/`showIeds` and changing `highlight`/`selectable` invalidates the base layers — the items most likely to be dropped from the key unnoticed. Test-only, low risk.
