...
- [ ] **Split `SldEditor`'s interaction lifecycle into explicit methods.** The `start*` methods still call the catch-all `reset()` — a holdover from when state lived in several fields. With the `InteractionState` union, assigning a new value already replaces the previous mode, but `reset()` _also_ cancels the placement promise and emits `sld-editor-in-action`. Separate the concerns into `setInteraction(...)`, `cancelInteraction()`, and `finishInteraction()` so replacement, cancellation, and completion read distinctly. Do together with item 6 (that event is emitted from this path).
