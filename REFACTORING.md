...
- [ ] **`<sld-context-menu>` actions still dispatch `EditV2`s directly — route them through the editor.** The context menu UI is now editor-owned, but `sld-context-menu-factory.ts` still dispatches ~26 edits directly. Per the edit-vs-view litmus test these belong in the **editor/controller** layer; in the eventual read-only `oscd-sld-viewer` package the viewer/menu must emit _intent_ events and let `oscd-sld-editor` build the `EditV2`. Behaviour-preserving for now; this is a boundary/architecture item, not a bug.
...
- [ ] Collapse artifact context interaction projections into interaction. SldSubstationViewer now owns one canonical interaction union, but artifact contexts still receive projected fields like placing, placingLabel, resizingBR, resizingTL, connecting, and idle. Migrate artifact contexts to consume interaction directly with mode narrowing, then delete redundant projection plumbing where possible. This should be done carefully across artifact specs because it touches many render/action helpers.
- [ ] Make the "Convert from old coordinates to new" screen a LOT nicer and user friendly. It should explain (perhaps in a read more... section, that previously coordinates were done handled _example..._ and that this was deemed at risk because other tools may not preserve this information, so mainly for data safety, it was decided to move all coordinates into a better protected <Private></Private> section, which in accordance to the iec 61850 specs, should always be preserved.
- [ ] Clarify SldEditor interaction lifecycle APIs. The start\* methods still call reset() because older state lived in multiple fields. With the discriminated union, assigning a new interaction already replaces the previous mode, but reset() also cancels placement promises and emits sld-editor-in-action. Split this into explicit methods such as setInteraction(...), cancelInteraction(), and finishInteraction() so replacement, cancellation, and completion are understandable.

- [ ] Replace or supplement sld-editor-in-action boolean event. The event detail is currently a bare boolean, which does not explain what changed or which interaction is active. Audit consumers first. If it is not public API, replace it with a typed interaction-change event. If compatibility matters, keep the boolean event temporarily and add a clearer event such as sld-editor-interaction-changed with { active, mode } or { interaction }.

- [ ] Route context-menu action edits through SldEditor. After the menu UI is editor-owned, sld-context-menu-factory.ts still dispatches direct EditV2s. Convert those actions to editor-owned intents in slices, starting with simple one-edit commands before compound actions like delete IED, delete busbar, and delete container.

- [ ] Continue large-file interaction performance work. The cached IED resolver removed the worst repeated full-document scan, and the closed IED menu no longer recomputes on every `docVersion`, but Bay move/undo still spends ~420-510 ms on the affected substation viewer render. Next likely direction: avoid rebuilding unchanged base SVG layers during mouse-driven interaction/doc-version refresh, or isolate moving/changed content from the heavy static diagram layers. Keep measuring with the Playwright large-file metric before and after each cut.
...
- **GUIDING PRINCIPLE for the viewer/editor split — the edit-vs-view litmus test.** When deciding where an operation belongs, ask: _does it produce an `EditV2` (mutate the SCL document)?_ → it belongs in the **editor/controller** layer (`SldEditor`). _Does it read/serialise the rendered view?_ → it belongs in the **view** layer (`SldSubstationViewer`). This is the rule that should drive the eventual `viewer / editor / plugin` module split. Confirmed state of the four `renderHeader` buttons against this test:
  ...
  Evidence the controller already owns the main gesture edits: `sld-editor.ts` handles `@oscd-sld-resize`/`-resize-tl`/`-place`/`-place-label`/`-connect`/`-rotate` and dispatches `newEditEventV2` (see `createResizeEdits`/`createResizeTLEdits` etc.). The remaining direct view/menu edit dispatches are tracked in the planned viewer/context-menu routing workstream.
