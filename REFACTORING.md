...
- [ ] **Interaction-state consolidation (`SldEditor` controller / `SldSubstationViewer` view) — DESIGN AGREED, READY TO IMPLEMENT.** The single highest-value structural item, and the future viewer/editor/plugin seam. Today the two components **duplicate the whole interaction-state model**: the same six gesture fields (`placing`, `placingOffset`, `placingLabel`, `resizingBR`, `resizingTL`, `connecting`), a byte-identical `interactionMode` getter, and the same inline `connecting` object shape — declared once as `@state` on `SldEditor` (the source of truth) and again as `@property` on `SldSubstationViewer` (a broadcast copy). `SldEditor.render()` then hand-binds all nine props down and wires thirteen gesture events back up. The target is a single owner of the interaction state, with the view receiving **one `.interaction` projection** instead of nine re-declared copies. Responsibilities (already reflected in the names):
  ...
  - **Agreed design (brainstorm settled):**
    - **State shape — discriminated union** keyed on `mode`, living in `foundations/interaction-mode.ts` (supersedes the `InteractionMode` string union, whose values become the `.mode` tags). Every active variant carries `element` as its subject; mode-specific extras hang off the variant. Makes illegal states unrepresentable and deletes the `interactionMode` getter:
      ...
      type Interaction =
        ...
        | { mode: "placingLabel"; element: Element }
      ...
      ```
      (`connecting` → `connectingFrom`; its `from` → `element`, `fromTerminal` → `terminal`; `path` stays as a mode-specific extra, NOT transient.)
    - **Ownership & reactivity — `@state() interaction` on `SldEditor` + pure transition functions in `foundations/interaction-mode.ts`** (e.g. `startPlacing(element, offset): Interaction`). The component just assigns the result, so Lit reactivity is automatic via the `@state` setter, and the transitions are unit-testable without a DOM (matching the repo convention of pure logic in `foundations/`, thin components). Mode exclusivity becomes free — assigning a new variant clears the others.
  ...
  - **Migration is behavior-preserving and guarded** by the existing exclusivity invariant test in `sld-editor.spec.ts`. Verify each step with `tsc --noEmit` + `npm run test` + `npm run format`.
  - Likely **subsumes** two smaller findings below (the dead `nsp` in the view; the inline edit dispatch), since both live on this controller/view boundary.
- [ ] **Remove the dead `nsp` property from the view / unify the defaults.** Three different defaults exist — root `'eosld'`, `SldEditor` `'eoscd'`, `SldSubstationViewer` `'esld'` — and neither the root nor `SldEditor` forwards `.nsp` to `SldSubstationViewer`, which also never detects it, so the view's `nsp` is permanently its default. **Not an active bug** (reads go through `getAttributeNS(sldNs, …)` by namespace URI, so the prefix is irrelevant for the view's read-only rendering; edits are built in the controller with the correct nsp), but it is a latent trap: a declared-but-dead property that would silently misbehave if an edit were ever built in the view. Remove it from the view (or wire/detect it honestly) and reconcile the defaults.
